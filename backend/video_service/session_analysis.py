...
class MultiShotDetector(ShootingAnalyzer):
  ...
  def _is_valid_shot_window(self, shot_window):
    ...
    if shot_length < MIN_SHOT_FRAMES or shot_length > MAX_SHOT_FRAMES:
      return False
  ...
  def reset_for_next_shot(self):
    saved_shots = self.shots
    saved_start_padding = self.start_padding
    saved_end_padding = self.end_padding

    ShootingAnalyzer.__init__(self, SHOOTING_ARM=self.SHOOTING_ARM, delta_t=self.delta_t)

    self.start_padding = saved_start_padding
    self.end_padding = saved_end_padding
    self.shots = saved_shots
    self.current_shot_start = None
...
def process_session_upload(file, user_id, shooting_arm="RIGHT"):
  ...
  try:
    ...
    for shot in detected_shots:
      shot_record = process_detected_shot(
        session_record=session_record,
        full_video_path=original_file_path,
    ...
    session_record.processing_error = None
    db.session.commit()
...
def process_detected_shot(session_record, full_video_path, session_hash, user_id, shot, shooting_arm):
  ...
  extract_frame_range_to_video(
    ...
    output_path=original_clip_path,
  ...
  return ShotAnalysis(
    ...
    video_url=shot_base_url + f"VIDEO_{shot_folder_name}.mp4",
    original_video_url=shot_base_url + original_clip_filename,
    setup_frame_url=shot_base_url + f"SETUP_{shot_folder_name}.png",
    release_frame_url=shot_base_url + f"RELEASE_{shot_folder_name}.png",
    follow_frame_url=shot_base_url + f"FOLLOW_{shot_folder_name}.png",
  )
...
def save_session_upload(file, user_id):
  ...
  original_filename = f"ORIGINAL_{session_hash}.mp4"
  original_path = os.path.join(session_folder_path, original_filename)
...
def get_shooting_side_landmarks(landmarks, shooting_arm):
  if shooting_arm == ShootingAnalyzer.ARM_LEFT:
    return {
...
def build_angles(analyzer, side_landmarks):
  ...
  knee = side_landmarks["knee"]
  ankle = side_landmarks["ankle"]
  eye = side_landmarks["eye"]
  ear = side_landmarks["ear"]

  return {
    "elbow": analyzer.calculate_angle(shoulder, elbow, wrist),
    "shoulder": analyzer.calculate_shoulder_angle(hip, shoulder, elbow),
    "wrist": analyzer.calculate_angle(elbow, wrist, index_finger),
    "body_lean": analyzer.calculate_body_lean(shoulder, hip),
    "hip_angle": analyzer.calculate_angle(shoulder, hip, knee),
    "dominant_knee": analyzer.calculate_angle(hip, knee, ankle),
    "head_tilt": analyzer.calculate_head_angle(eye, ear),
    "forearm_alignment": analyzer.calculate_forearm_alignment(elbow, wrist),
    "setpoint": analyzer.calculate_release_setpoint(index_finger, eye),
  }
...
def extract_frame_range_to_video(input_path, output_path, start_frame, end_frame, overlay_frame_numbers=False):
  ...
  safe_end = min(end_frame, max(0, total_frames - 1))
  ...
  current_frame = safe_start
  font = cv2.FONT_HERSHEY_SIMPLEX

  try:
    while current_frame <= safe_end:
      ret, frame = cap.read()
      if not ret:
        break

      if overlay_frame_numbers:
        cv2.putText(frame, f"Source Frame: {current_frame}", (20, 40), font, 1, (255, 255, 255), 2)
        cv2.putText(frame, f"Clip Range: {safe_start} - {safe_end}", (20, 80), font, 0.9, (0, 255, 0), 2)
